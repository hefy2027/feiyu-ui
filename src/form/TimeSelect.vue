...
<template>
  ...
  >
    <div class="ui-time-select__input-box" @click="toggleDropdown">
      <span v-if="prefixIcon" class="material-symbols-outlined ui-time-select__icon">
        {{ prefixIcon }}
      </span>

      <span v-if="modelValue" class="ui-time-select__value">{{ modelValue }}</span>
      <span v-else class="ui-time-select__placeholder">{{ placeholder }}</span>

      <span
        v-if="clearable && modelValue && !disabled"
        class="material-symbols-outlined ui-time-select__clear"
        title="清空"
        @click="handleClear"
      >
        cancel
      </span>
      <span v-else class="material-symbols-outlined ui-time-select__arrow">
        arrow_drop_down
      </span>
    </div>

    <!-- Dropdown Menu -->
    <transition name="ui-time-select-fade">
      <div v-if="isOpen" ref="listRef" class="ui-time-select__dropdown">
        <div
          v-for="item in timeOptions"
          :key="item.value"
          :class="[
            'ui-time-select__item',
            {
        ...
        >
          ...
          <span v-if="modelValue === item.value" class="material-symbols-outlined ui-time-select__check">
            check
          </span>
        </div>
      </div>
...
.ui-time-select-fade-enter-active,
.ui-time-select-fade-leave-active {
  transition: opacity 0.18s var(--ease-soft), transform 0.18s var(--ease-soft);
}
.ui-time-select-fade-enter-from,
.ui-time-select-fade-leave-to {
  opacity: 0;
