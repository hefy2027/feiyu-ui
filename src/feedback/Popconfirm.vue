...
const props = withDefaults(defineProps<Props>(), {
  ...
  icon: 'help',
  iconColor: 'var(--theme-yellow, #f59e0b)',
  placement: 'top',
  disabled: false
})
...
<template>
  <div ref="popconfirmRef" class="ui-popconfirm">
    <div class="ui-popconfirm__trigger" @click.stop="toggle">
      <slot />
    </div>

    <transition name="ui-popconfirm-fade">
      <div
        v-if="isOpen"
        :class="[
          'ui-popconfirm__popover',
          `ui-popconfirm__popover--${placement}`
        ]"
        @click.stop
      >
        <div class="ui-popconfirm__main">
          <span
            v-if="icon"
            class="material-symbols-outlined ui-popconfirm__icon"
            :style="{ color: iconColor }"
          >
            {{ icon }}
          </span>
          <div class="ui-popconfirm__message">
...
.ui-popconfirm-fade-enter-active,
.ui-popconfirm-fade-leave-active {
  transition: opacity 0.18s var(--ease-soft), transform 0.18s var(--ease-out);
}
.ui-popconfirm-fade-enter-from,
.ui-popconfirm-fade-leave-to {
  opacity: 0;
